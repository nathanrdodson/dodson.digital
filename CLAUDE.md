...
dodson.digital is the personal counterpart to [dodson.mba](https://dodson.mba) (career/professional site, separate repo). This repo carries all personal writing, photography, and travel content, and was forked from dodson.mba's codebase — same Astro setup, same conventions below, minus the career-only pages.
...
This is the repo's main footgun. A `/images/...` path can be referenced either as a frontmatter field (`featureImage`) **or** as an entry in the `images:` array of a `src/content/photos/*.md` album. dodson.mba lost 16 live photos to a cleanup that scanned only `featureImage` — don't repeat it here.
...
`public/images/` is ~176MB and is the only directory with unbounded growth — every new album/post adds to it. See `PERF-1` in ISSUES.md.
...
`src/pages/design.astro` renders live design tokens and every shared component against real collection content, at `https://dodson.digital/design/`. It is unlinked from the nav but publicly reachable and **is not a scratch page**. When changing a design token, adding a component, or altering component markup, update `/design` in the same commit.

Note: this page was forked from dodson.mba's identical `/design` and still shares its component library. If you change a primitive shared with dodson.mba (buttons, cards, timeline, hero), consider whether the same fix belongs in that repo too — they will drift silently otherwise.
...
[ISSUES.md](ISSUES.md) is the backlog of known problems and planned improvements, with stable IDs (`PERF-1`, `SEO-2`, …), measured evidence, and a Resolved log. It was seeded from dodson.mba's backlog for the issues that moved here with the content — re-measure rather than trusting the inherited numbers, since they were taken against the old repo's build.
